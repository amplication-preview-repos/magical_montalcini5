/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { UserChatRoomWhereUniqueInput } from "./UserChatRoomWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { UserChatRoomUpdateInput } from "./UserChatRoomUpdateInput";

@ArgsType()
class UpdateUserChatRoomArgs {
  @ApiProperty({
    required: true,
    type: () => UserChatRoomWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => UserChatRoomWhereUniqueInput)
  @Field(() => UserChatRoomWhereUniqueInput, { nullable: false })
  where!: UserChatRoomWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => UserChatRoomUpdateInput,
  })
  @ValidateNested()
  @Type(() => UserChatRoomUpdateInput)
  @Field(() => UserChatRoomUpdateInput, { nullable: false })
  data!: UserChatRoomUpdateInput;
}

export { UpdateUserChatRoomArgs as UpdateUserChatRoomArgs };
